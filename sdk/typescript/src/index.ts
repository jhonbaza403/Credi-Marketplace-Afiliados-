...
export class CrediClient{
 ...
 private async request<T>(path:string,init:RequestInit={}):Promise<T>{const r=await fetch(`${this.baseUrl}${path}`,{...init,headers:{Authorization:`Bearer ${this.apiKey}`,'Content-Type':'application/json',...(init.headers||{})}});const d=await r.json().catch(()=>({})) as {error?: unknown};if(!r.ok)throw new Error(typeof d.error==='string'?d.error:`Credi API ${r.status}`);return d as T}
