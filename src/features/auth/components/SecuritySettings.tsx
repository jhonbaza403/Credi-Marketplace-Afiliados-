...
export default function SecuritySettings() {
  ...
  const [passkeys, setPasskeys] = useState<Array<{ id: string; friendly_name?: string | null; created_at?: string; last_used_at?: string | null }>>([]);
  ...
  async function loadSecurityState() {
    ...
    try {
      ...
      setPasskeys((passkeyData ?? []) as typeof passkeys);
    } catch (error) {
  ...
  async function addPasskey() {
    ...
    try {
      const { error } = await supabase.auth.registerPasskey({
        friendlyName: typeof navigator !== 'undefined' && navigator.userAgent.includes('Windows') ? 'Este dispositivo — Windows Hello' : 'Este dispositivo',
      });
      if (error) throw error;
  ...
  return (
    <section className="mx-auto w-full max-w-3xl space-y-6">
      <header className="rounded-3xl border border-border bg-card p-7 shadow-sm sm:p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">Puedes usar autenticación en dos pasos con una aplicación autenticadora y registrar llaves de acceso para tus dispositivos.</p>
      </header>
      ...
      <section className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8" aria-labelledby="mfa-title">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">Usa una aplicación autenticadora para generar un código temporal de 6 dígitos al iniciar sesión.</p>

        {loading ? <p className="mt-5 text-sm text-muted-foreground">Cargando configuración...</p> : (
          <>
            {factors.filter((factor) => factor.factor_type === 'totp' && factor.status === 'verified').length > 0 && (
              <p className="mt-5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-sm font-semibold text-emerald-700 dark:text-emerald-300">2FA TOTP está activo en esta cuenta.</p>
            )}
            {!factorId && factors.filter((factor) => factor.factor_type === 'totp' && factor.status === 'verified').length === 0 && (
              <button type="button" onClick={startTotp} disabled={working} className="mt-5 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50">Activar 2FA</button>
            )}
            {factorId && qrCode && (
              <div className="mt-5 space-y-4 rounded-2xl border border-border bg-background p-5">
                ...
                <label className="block text-sm font-semibold text-foreground">Código de verificación<input value={totpCode} onChange={(event) => setTotpCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" maxLength={6} pattern="[0-9]{6}" aria-label="Código de verificación de autenticación en dos pasos" className="mt-2 w-full rounded-xl border border-border bg-card px-4 py-3 tracking-[0.3em] outline-none focus:ring-2 focus:ring-primary/20" /></label>
                <button type="button" onClick={verifyTotp} disabled={working || totpCode.length !== 6} className="w-full rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50">Verificar y activar</button>
              </div>
            )}
          </>
        )}
      ...
      <section className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8" aria-labelledby="passkey-title">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">Registra Windows Hello, biometría del dispositivo, PIN del dispositivo o una llave de seguridad compatible. La clave privada permanece en tu autenticador.</p>
        <button type="button" onClick={addPasskey} disabled={working} className="mt-5 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50">Registrar este dispositivo</button>
        <div className="mt-6 space-y-3">
          {passkeys.length === 0 ? <p className="text-sm text-muted-foreground">No hay llaves de acceso registradas.</p> : passkeys.map((passkey) => (
            <div key={passkey.id} className="flex flex-col gap-3 rounded-2xl border border-border bg-background p-4 sm:flex-row sm:items-center sm:justify-between">
              <div><p className="font-bold text-foreground">{passkey.friendly_name || 'Dispositivo'}</p><p className="text-xs text-muted-foreground">Registrado: {passkey.created_at ? new Date(passkey.created_at).toLocaleString() : '—'}</p><p className="text-xs text-muted-foreground">Último uso: {passkey.last_used_at ? new Date(passkey.last_used_at).toLocaleString() : 'Aún no utilizado'}</p></div>
              <button type="button" onClick={() => removePasskey(passkey.id)} disabled={working} className="rounded-lg border border-border px-3 py-2 text-sm font-bold text-foreground hover:bg-muted disabled:opacity-50">Eliminar</button>
            </div>
          ))}
        </div>
      </section>
