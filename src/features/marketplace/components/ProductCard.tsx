...
function formatPrice(price: number, currency = 'USD'): string {
  ...
  try {
    return new Intl.NumberFormat('es-VE', { style: 'currency', currency, minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(price);
  } catch {
    return `${currency} ${price.toFixed(2)}`;
  }
}

export function ProductCard({ product, onAddToCart, disabled = false, showBuyButton = true }: ProductCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl focus-within:ring-2 focus-within:ring-blue-500">
      <Link href={productHref} aria-label={`Ver ${product.title}`} className="relative block aspect-square overflow-hidden bg-slate-100">
        {image ? <Image src={image} alt={product.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-105" unoptimized={image.startsWith('http') && !image.includes('.supabase.co/')} /> : <div className="flex h-full items-center justify-center text-sm font-semibold text-slate-400" aria-label="Producto sin imagen">Sin imagen</div>}
        {!isAvailable && <div className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-3 py-2 text-center text-xs font-semibold uppercase tracking-wide text-white">{product.stock <= 0 ? 'Agotado' : 'No disponible'}</div>}
      </Link>

      <div className="flex flex-1 flex-col p-5">
        <Link href={productHref} className="line-clamp-2 text-lg font-bold leading-6 text-slate-900 hover:text-blue-600">{product.title}</Link>
        {product.description && <p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-500">{product.description}</p>}

        <div className="mt-3 min-h-5">
          {isAvailable ? <p className="text-xs font-semibold text-emerald-600">{product.stock <= 5 ? `Últimas ${product.stock} unidades` : 'Disponible'}</p> : <p className="text-xs font-medium text-slate-400">No disponible</p>}
        </div>
