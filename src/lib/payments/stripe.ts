...
async function stripePost<T>(
  ...
  if (!response.ok) {
    const message = payload && typeof payload === 'object' && 'error' in payload
      ? String((payload as { error?: { message?: unknown } }).error?.message ?? 'Stripe API error')
      : `Stripe API respondió ${response.status}.`
...
export async function createStripeCheckoutSession(input: {
  ...
  currency: string
  amountMinor: number
  lineItems: Array<{ name: string; quantity: number; unitAmountMinor: number }>
  successUrl: string
  cancelUrl: string
  idempotencyKey: string
  ...
  params.set('metadata[platform]', 'credi-marketplace')
  ...
  input.lineItems.forEach((item, index) => {
    ...
    params.set(`line_items[${index}][price_data][product_data][name]`, item.name.slice(0, 500))
  })

  const session = await stripePost<StripeCheckoutSession>('/checkout/sessions', params, input.idempotencyKey)

  if (!session.id || !session.url) throw new Error('Stripe no devolvió una URL de checkout válida.')
  if (session.amount_total !== input.amountMinor) throw new Error('El importe confirmado por Stripe no coincide con la orden.')
  if (session.currency?.toUpperCase() !== input.currency.toUpperCase()) throw new Error('La moneda confirmada por Stripe no coincide con la orden.')
