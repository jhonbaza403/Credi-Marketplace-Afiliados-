...
export default function B2BProductPublisher() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/b2b/products", {
        ...
        credentials: "include",
        body: JSON.stringify({
          title: form.title.trim(), category: form.category, description: form.description.trim(), wholesale: Number(form.wholesalePrice), regular: Number(form.regularPrice), moq: Number(form.minOrder), stock: Number(form.stock), country: form.country.trim().toUpperCase(), binancePayId: form.binancePayId.trim(), usdtWalletAddress: form.usdtWallet.trim(), images: images.map((item) => ({ url: item.url, kind: item.kind })), videos: videoMedia,
        }),
      })
      ...
      setImages([]); setVideos([]); router.refresh()
    } catch (error: unknown) {
  ...
  return <section className="rounded-[2rem] border border-cyan-300/10 bg-[#07101f]/90 p-6 text-white shadow-[0_30px_100px_rgba(2,8,28,.35)] sm:p-8">
    ...
    <form onSubmit={submit} className="space-y-7">
      <div className="grid gap-5 lg:grid-cols-2"><Field label="Nombre del producto" value={form.title} onChange={(value) => update("title", value)} required /><label className="text-sm font-bold">Categoría<select value={form.category} onChange={(event) => update("category", event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 outline-none focus:border-cyan-300/50">{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></label></div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4"><Field label="Precio mayorista USD" type="number" min="0.01" step="0.01" value={form.wholesalePrice} onChange={(value) => update("wholesalePrice", value)} required /><Field label="Precio de referencia USD" type="number" min="0.01" step="0.01" value={form.regularPrice} onChange={(value) => update("regularPrice", value)} required /><Field label="Pedido mínimo" type="number" min="1" step="1" value={form.minOrder} onChange={(value) => update("minOrder", value)} required /><Field label="Stock disponible" type="number" min="1" step="1" value={form.stock} onChange={(value) => update("stock", value)} required /></div>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><Field label="País del proveedor (ISO-2)" value={form.country} maxLength={2} onChange={(value) => update("country", value.toUpperCase())} required /><Field label="Binance Pay ID (opcional)" value={form.binancePayId} onChange={(value) => update("binancePayId", value)} /><Field label="Wallet USDT TRC20 (opcional)" value={form.usdtWallet} onChange={(value) => update("usdtWallet", value)} /></div>
      <div className="grid gap-5 lg:grid-cols-2"><MediaBlock title="Imagen principal" note="JPG, PNG, WebP o GIF · hasta 20 MB" kind="image" value={images} onChange={setImages} maxFiles={1} /><MediaBlock title="Vídeo del lote (opcional)" note="MP4, WebM o MOV · hasta 500 MB" kind="video" value={videos} onChange={setVideos} maxFiles={1} /></div>
...
function Field({ label, value, onChange, type = "text", required = false, maxLength = 240, min, step }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; maxLength?: number; min?: string; step?: string }) {
