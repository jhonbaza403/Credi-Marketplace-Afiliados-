...
export default function AffiliateLinksWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!affiliate) { setAffiliateId(null); return }
  ...
  function getLink(code: string) { return `${CANONICAL_APP_URL}/products?ref=${encodeURIComponent(code)}` }

  return <main className="min-h-screen bg-[#050816] text-white"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    <header className="overflow-hidden rounded-[2rem] border border-cyan-300/10 bg-[radial-gradient(circle_at_10%_10%,rgba(34,211,238,.15),transparent_30%),radial-gradient(circle_at_90%_0%,rgba(168,85,247,.16),transparent_32%),linear-gradient(135deg,#0b1024,#060914)] p-7 shadow-[inset_0_1px_0_rgba(255,255,255,.08),0_30px_100px_rgba(0,0,0,.3)] sm:p-10"><span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[.18em] text-cyan-200">Affiliate Commerce Studio</span><h1 className="mt-4 text-3xl font-black sm:text-5xl">Enlaces por producto</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300">Un solo centro para crear enlaces, definir comisiones especiales y vigilar el rendimiento de cada producto.</p></header>
    ...
    {!affiliateId ? <section className="mt-6 rounded-3xl border border-amber-300/15 bg-amber-300/5 p-6 text-sm text-amber-100">Tu perfil de afiliado aún no está activo. Primero completa la solicitud del programa.</section> : <>
      ...
      {loading ? <div className="mt-6 h-48 animate-pulse rounded-2xl bg-white/5" /> : <div className="mt-6 grid gap-4">{filtered.map((product) => { const link = linkMap.get(product.id); const commission = Number((link?.commission_rate_override ?? rate) * 100); return <article key={product.id} className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><h3 className="truncate text-sm font-black text-white">{product.title}</h3><p className="mt-1 text-xs text-slate-400">${Number(product.price).toFixed(2)} · {commission.toFixed(2)}% por defecto</p></div>{link ? <div className="grid gap-3 lg:grid-cols-[1fr_auto_auto]"><input readOnly value={getLink(link.code)} className="min-w-0 rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2.5 text-xs text-slate-300"/><div className="flex items-center gap-2"><span className="text-[10px] font-black text-slate-500">COMISIÓN %</span><input defaultValue={commission.toFixed(2)} onBlur={(e) => void saveRate(link, e.target.value)} className="w-24 rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2.5 text-xs text-white"/></div><button type="button" onClick={() => void navigator.clipboard.writeText(getLink(link.code))} className="rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-black text-slate-950">Copiar</button></div> : <button type="button" disabled={busy === product.id} onClick={() => void makeLink(product)} className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-2.5 text-xs font-black text-cyan-100 disabled:opacity-50">{busy === product.id ? "Creando…" : "Crear enlace"}</button>}</div>{link && <div className="mt-3 flex flex-wrap gap-5 text-[11px] text-slate-400"><span>Clicks <b className="text-white">{link.clicks}</b></span><span>Conversiones <b className="text-white">{link.conversions}</b></span><span>Estado <b className="text-emerald-200">Activo</b></span></div>}</article> })}</div>}
