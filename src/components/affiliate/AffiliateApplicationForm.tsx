...
export default function AffiliateApplicationForm() {
  ...
  function toggleMethod(item: string) {
    setMethods((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item])
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error } = await supabase.from("affiliate_applications").insert({
        ...
        social_profiles: { instagram: form.instagram.trim(), tiktok: form.tiktok.trim(), youtube: form.youtube.trim() },
        promotion_methods: methods,
        tax_residency_country: form.taxCountry || null,
        tax_id: form.taxId.trim() || null,
        terms_version: TERMS_VERSION,
        privacy_version: TERMS_VERSION,
        affiliate_policy_version: TERMS_VERSION,
  ...
  return (
    <form onSubmit={submit} className="rounded-[2rem] border border-white/10 bg-white/[.035] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,.06),0_24px_70px_rgba(2,8,28,.28)] sm:p-8">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-bold text-slate-100">País<select value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })} className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white"><option value="US">Estados Unidos</option><option value="VE">Venezuela</option><option value="CO">Colombia</option><option value="MX">México</option><option value="ES">España</option><option value="OTHER">Otro</option></select></label>
      ...
      {status && <p role="status" className="mt-5 rounded-2xl border border-cyan-300/15 bg-cyan-300/5 p-4 text-sm text-cyan-100">{status}</p>}
