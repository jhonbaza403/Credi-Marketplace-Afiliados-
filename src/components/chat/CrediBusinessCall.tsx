...
export default function CrediBusinessCall({
  conversationId,
  currentUserId,
  peerUserId,
  peerName,
  ...
  const finishCall = useCallback(async (callId: string | null, status: CallRow['status'] = 'ended') => {
    if (callId) {
      ...
      if (currentUserId) {
        await supabase.from('chat_call_participants').update({ state: status === 'declined' ? 'declined' : 'left', left_at: new Date().toISOString() }).eq('call_id', callId).eq('user_id', currentUserId)
        if (peerRef.current) {
  ...
  const preparePeer = useCallback(async (callRow: CallRow, remoteUserId: string, offerer: boolean) => {
    const media = await navigator.mediaDevices.getUserMedia({ audio: true, video: callRow.call_type === 'video' })
    ...
    connection.onicecandidate = (event) => {
      if (event.candidate) void sendSignal(callRow.id, remoteUserId, 'ice', { candidate: event.candidate.toJSON() }).catch(() => undefined)
    }
    ...
    if (offerer) {
      ...
      await sendSignal(callRow.id, remoteUserId, 'offer', { description: offer })
    }
    return media
  }, [sendSignal, supabase])
  ...
  const acceptCall = useCallback(async (incomingCall: CallRow) => {
    ...
    try {
      ...
      await supabase.from('chat_call_participants').update({ state: 'accepted', joined_at: new Date().toISOString() }).eq('call_id', incomingCall.id).eq('user_id', currentUserId)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_call_signals', filter: `recipient_id=eq.${currentUserId}` }, async (payload) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_calls', filter: `conversation_id=eq.${conversation}` }, (payload) => {
        const next = payload.new as CallRow
        if (next.initiated_by !== user && next.status === 'ringing') setIncoming((current) => current ?? next)
      })
      .subscribe()
    return () => { void supabase.removeChannel(channel) }
