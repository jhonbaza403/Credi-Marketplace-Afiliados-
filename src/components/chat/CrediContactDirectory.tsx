...
export default function CrediContactDirectory() {
  ...
  async function searchContact() {
    ...
    setBusy(true); setError(null); setNotice(null); setContact(null)
    const supabase = createClient()
    const { data, error: rpcError } = await supabase.rpc('get_credi_contact_by_pin', { p_credi_pin: normalized })
    if (rpcError) setError(rpcError.message || 'No fue posible buscar el contacto.')
    else if (data?.[0]) setContact(data[0] as Contact)
    else setError('No encontramos un participante activo con ese PIN Credi.')
    setBusy(false)
  }

  function copyPin() {
    if (!myPin) return
    void navigator.clipboard.writeText(myPin).then(() => { setNotice('PIN copiado al portapapeles.') }).catch(() => setNotice('PIN: ' + myPin))
  }
    ...
    <section className="mb-4 rounded-[1.75rem] border border-cyan-300/10 bg-[#08101f] p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-lg font-black text-white sm:text-xl">Conecta por PIN Credi</h2>
          <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-400">Busca vendedores, proveedores, empresarios, afiliados y otros participantes sin publicar su correo o teléfono.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
        <div className="relative"><Search className="absolute left-3 top-3 size-4 text-slate-500" /><input value={pin} onChange={(event) => setPin(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === 'Enter') void searchContact() }} placeholder="Ej.: CRD-A1B2C3D4E5" maxLength={14} className="w-full rounded-xl border border-white/10 bg-white/[.04] py-3 pl-9 pr-3 text-sm font-semibold tracking-[.08em] text-white outline-none placeholder:text-slate-500" aria-label="PIN Credi del contacto" /></div>
        <button type="button" onClick={() => void searchContact()} disabled={busy || !pin.trim()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-40"><Search size={16} /> {busy ? 'Buscando…' : 'Buscar contacto'}</button>
      </div>
