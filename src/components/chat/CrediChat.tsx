...
export default function CrediChat() {
  ...
  async function sendAttachment(file: File) {
    ...
    try {
      ...
      const type = media.kind === 'video' ? 'video' : media.kind === 'audio' ? 'audio' : 'image'
  ...
  return (
    <main className="credi-chat-page min-h-[calc(100vh-80px)]">
      <div className="mx-auto max-w-7xl px-3 py-4 sm:px-6 sm:py-8">
        <div className="mb-4 rounded-[1.75rem] border border-cyan-300/10 credi-chat-hero p-5 sm:p-7">
          <div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl credi-chat-icon credi-chat-accent"><MessageCircle size={20} /></span><div><p className="text-[10px] font-black uppercase tracking-[.18em] credi-chat-accent">Credi Business Chat</p><h1 className="text-2xl font-black sm:text-3xl">Comunicación comercial integrada</h1></div></div>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--foreground)]">Habla con compradores, vendedores y proveedores sin salir de Credi Marketplace. Las conversaciones pueden quedar vinculadas a productos, B2B y pedidos.</p>
        </div>
        ...
        <div className="grid min-h-[620px] overflow-hidden rounded-[2rem] border border-white/10 credi-chat-panel shadow-2xl lg:grid-cols-[340px_1fr]">
          <aside className="border-b border-white/10 credi-chat-sidebar lg:border-b-0 lg:border-r">
            <div className="border-b border-white/10 p-4"><div className="relative"><Search className="absolute left-3 top-3.5 size-4 text-slate-500"/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Buscar conversaciones" className="w-full rounded-xl border border border-[var(--border)] credi-chat-input py-3 pl-9 pr-3 text-sm text-white outline-none placeholder:text-[var(--muted-light)]"/></div></div>
            <div className="max-h-[280px] overflow-y-auto lg:max-h-[540px]">{loading ? <div className="p-6 text-sm credi-chat-muted">Cargando Credi Chat…</div> : filtered.length ? filtered.map((item) => <button key={item.id} type="button" onClick={()=>{setSelectedId(item.id); router.replace(`/chat?conversation=${encodeURIComponent(item.id)}`)}} className={`w-full border-b border-white/5 px-4 py-4 text-left transition hover:bg-white/[.04] ${selectedId===item.id?'bg-[color-mix(in_oklab,var(--primary)_8%,transparent)]':''}`}><div className="flex items-start gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-300/20 to-violet-400/20 text-xs font-black text-cyan-100">{item.display_name.slice(0,1).toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-black text-white">{item.display_name}</p><span className="text-[10px] text-slate-500">{new Date(item.updated_at).toLocaleDateString('es-VE')}</span></div><p className="mt-1 truncate text-xs credi-chat-muted">{item.title || 'Conversación comercial'}</p></div></div></button>) : <div className="p-6 text-sm leading-6 credi-chat-muted">Aún no tienes conversaciones. Desde un producto o una oferta B2B pulsa <b className="credi-chat-accent">Contactar</b> para iniciar una.</div>}</div>
          </aside>

          <section className="flex min-h-[620px] flex-col credi-chat-panel">
            {selected ? <>
              <header className="flex items-center gap-3 border-b border-white/10 px-5 py-4"><div className="flex size-10 items-center justify-center rounded-full bg-gradient-to-br from-cyan-300/20 to-violet-400/20 font-black text-cyan-100">{(selected.display_name||'C').slice(0,1).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-black">{selected.display_name}</p><p className="truncate text-xs credi-chat-muted">{selected.title || 'Canal comercial protegido'}</p></div>{otherId && profiles[otherId]?.role && <span className="ml-auto rounded-full border border border-[var(--border)] credi-chat-input px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--foreground)]">{profiles[otherId].role}</span>}</header>
              <div ref={messageBox} className="flex-1 space-y-3 overflow-y-auto p-4 sm:p-6">
                {messages.length ? messages.map((message) => { const mine = message.sender_id === userId; const meta = message.metadata || {}; const mediaUrl = typeof meta.public_url === 'string' ? meta.public_url : null; return <div key={message.id} className={`flex ${mine?'justify-end':'justify-start'}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm shadow-sm ${mine?'rounded-br-md credi-chat-message-own':'rounded-bl-md border border-white/10 bg-white/[.05] text-slate-100'}`}>{message.message_type==='image' && mediaUrl ? <img src={mediaUrl} alt="Archivo compartido" className="mb-2 max-h-72 rounded-xl object-cover"/> : null}{message.message_type==='video' && mediaUrl ? <video src={mediaUrl} controls className="mb-2 max-h-72 rounded-xl"/> : null}{message.message_type==='audio' && mediaUrl ? <audio src={mediaUrl} controls className="mb-2 w-full max-w-sm"/> : null}{message.body && <p className="whitespace-pre-wrap leading-6">{message.body}</p>}{message.message_type==='image' && !mediaUrl && <p>Imagen compartida</p>}{message.message_type==='video' && !mediaUrl && <p>Vídeo compartido</p>}{message.message_type==='audio' && !mediaUrl && <p>Audio compartido</p>}<div className={`mt-1 text-[9px] ${mine?'text-slate-700':'text-slate-500'}`}>{new Date(message.created_at).toLocaleTimeString('es-VE',{hour:'2-digit',minute:'2-digit'})}</div></div></div> }) : <div className="flex h-full items-center justify-center text-center text-sm text-slate-500"><div><MessageCircle className="mx-auto mb-3 size-8 text-cyan-300/40"/><p>Inicia la conversación.</p><p className="mt-1 text-xs">Pregunta por precio, disponibilidad, MOQ, envío o condiciones comerciales.</p></div></div>}
              </div>
              <div className="border-t border-white/10 credi-chat-sidebar p-3 sm:p-4"><div className="flex items-end gap-2"><label className="flex size-11 shrink-0 cursor-pointer items-center justify-center rounded-xl border border border-[var(--border)] credi-chat-input text-[var(--foreground)] hover:bg-white/[.08]" aria-label="Adjuntar imagen, vídeo o audio"><Paperclip size={18}/><input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime,audio/aac,audio/flac,audio/m4a,audio/mp4,audio/mpeg,audio/ogg,audio/opus,audio/webm,audio/wav,audio/x-m4a,audio/x-wav" className="sr-only" disabled={uploading} onChange={(e)=>{const file=e.target.files?.[0];if(file) void sendAttachment(file);e.currentTarget.value=''}}/></label><button type="button" className="hidden size-11 items-center justify-center rounded-xl border border border-[var(--border)] credi-chat-input text-[var(--foreground)] hover:bg-white/[.08] sm:flex" aria-label="Añadir emoji"><Smile size={18}/></button><textarea value={draft} onChange={(e)=>setDraft(e.target.value)} onKeyDown={(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void sendText()}}} rows={1} placeholder="Escribe un mensaje comercial…" className="max-h-32 min-h-11 flex-1 resize-y rounded-xl border border border-[var(--border)] credi-chat-input px-4 py-3 text-sm text-white outline-none placeholder:text-[var(--muted-light)]"/><button type="button" onClick={()=>void sendText()} disabled={sending||uploading||!draft.trim()} className="flex size-11 shrink-0 items-center justify-center rounded-xl credi-chat-message-own disabled:cursor-not-allowed disabled:opacity-40" aria-label="Enviar mensaje"><Send size={18}/></button></div><p className="mt-2 flex items-center gap-1 text-[10px] text-slate-500"><Mic size={12} />{uploading?'Subiendo archivo…':'Adjunta audio de hasta 500 MB · Enter envía · Shift+Enter agrega una línea'}</p></div>
            </> : <div className="flex flex-1 items-center justify-center p-8 text-center"><div><MessageCircle className="mx-auto mb-4 size-12 text-cyan-300/30"/><h2 className="text-xl font-black">Credi Chat</h2><p className="mt-2 max-w-md text-sm leading-6 credi-chat-muted">Selecciona una conversación o entra a un producto/B2B para contactar directamente al comerciante.</p></div></div>}
          </section>
        </div>
      </div>
    </main>
