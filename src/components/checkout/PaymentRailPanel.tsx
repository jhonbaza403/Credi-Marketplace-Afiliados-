...
type Result = { ok?: boolean; error?: string; next_action?: string; checkout?: { url?: string }; rail?: Record<string, unknown>; payment?: { provider_reference?: string; status?: string; amount?: number; currency?: string } }
...
export default function PaymentRailPanel() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !data.ok && !data.checkout?.url) throw new Error(data.error || `No fue posible iniciar ${label}.`)
      setResult(data)
      if (method === 'stripe' && data.checkout?.url) window.location.assign(data.checkout.url)
  ...
  return <section className="mt-8 rounded-3xl border border-border bg-background/80 p-6 shadow-xl">
    ...
    <div className="mt-5 grid gap-2 sm:grid-cols-5">{rails.map(([key, name]) => <button key={key} type="button" onClick={() => { setMethod(key); setResult(null) }} className={`rounded-xl border p-3 text-left text-sm font-black transition ${method === key ? 'border-primary bg-primary/10' : 'border-border bg-muted/20'}`}><span>{name}</span><span className="mt-1 block text-[11px] font-medium text-muted-foreground">{key === 'stripe' ? 'Checkout automático' : key === 'wallet' ? 'Liquidación interna' : 'Verificación de pago'}</span></button>)}</div>
