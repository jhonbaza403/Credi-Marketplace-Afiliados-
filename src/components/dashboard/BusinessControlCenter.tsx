...
export default function BusinessControlCenter() {
  ...
  async function toggleCatalogItem(productId: string) {
    ...
    const { error: addError } = await supabase
      .from('business_catalog_items')
      .insert({ catalog_id: selectedCatalog, product_id: productId, owner_id: user.id })
  ...
  async function publishCatalog() {
    ...
    const { error: updateError } = await supabase
      .from('business_catalogs')
      .update({ status: 'published', visibility: 'marketplace' })
      .eq('id', selectedCatalog)
      .eq('owner_id', user.id)

    if (updateError) return setError(updateError.message)
    setCatalogs((items) => items.map((catalog) => (catalog.id === selectedCatalog ? { ...catalog, status: 'published', visibility: 'marketplace' } : catalog)))
    setMessage('Catálogo publicado en Marketplace. Cada producto conserva sus propios controles.')
  }
  ...
  async function createPost() {
    ...
    const { error: postError } = await supabase.from('feed_posts').insert({
      owner_id: user.id,
      body: postText.trim(),
      product_id: selectedProductForContent || null,
    ...
    setPostText('')
    setMessage('Publicación creada y enviada a moderación.')
  }
  ...
  async function createStory() {
    ...
    setStoryText('')
    setMessage('Historia creada y enviada a moderación.')
  }
      ...
      <main className="min-h-screen bg-background p-6">
        <div className="mx-auto flex min-h-[60vh] max-w-7xl items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Cargando centro empresarial…</p>
          </div>
        </div>
      </main>
    )
    ...
    return (
      <main className="min-h-screen bg-background p-6">
        <div className="mx-auto max-w-3xl rounded-3xl border border-border bg-card p-10 text-center">
          ...
          <p className="mt-3 text-muted-foreground">Esta área está disponible para cuentas comerciales.</p>
          <Link className="mt-6 inline-flex rounded-xl bg-primary px-5 py-3 font-bold text-primary-foreground" href="/dashboard/profile">Configurar mi cuenta</Link>
        </div>
      </main>
    )
  }
        ...
        <section className="mt-7 grid gap-6 xl:grid-cols-[1.6fr_.9fr]">
          <div className="rounded-3xl border border-border bg-card p-5 sm:p-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[.18em] text-primary">Control total</p>
                <h2 className="mt-1 text-xl font-black">Inventario y publicación</h2>
              </div>
              ...
              <table className="w-full min-w-[980px] text-left text-sm">
                <thead className="bg-muted/50">
                  <tr className="border-b border-border">
                    {['Producto', 'Stock', 'Catálogo', 'Marketplace', 'B2B', 'Feed', 'Historia', 'Venta'].map((head) => (
                      <th key={head} className="px-4 py-3 text-[10px] font-black uppercase tracking-wider text-muted-foreground">{head}</th>
                    ))}
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-muted/20">
                        <td className="max-w-[280px] px-4 py-4">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">${Number(product.price).toFixed(2)}</p>
                        </td>
                        <td className="px-4 py-4">
                          <input aria-label={`Stock de ${product.title}`} type="number" min="0" value={product.stock} onChange={(event) => void updateStock(product.id, event.target.value)} className="w-24 rounded-lg border border-border bg-background px-2 py-1.5" />
                        </td>
                        {CONTROL_KEYS.map((key) => {
                          const active = Boolean(control?.[key])
                          return (
                            <td key={key} className="px-4 py-4">
                              <button type="button" aria-pressed={active} onClick={() => void toggle(product.id, key)} className={`rounded-full px-3 py-1 text-xs font-black ${active ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
          ...
          <aside className="space-y-6">
            <section className="rounded-3xl border border-border bg-card p-6">
              ...
              <div className="mt-5 flex gap-2">
                <input value={catalogName} onChange={(event) => setCatalogName(event.target.value)} placeholder="Nombre del catálogo" className="min-w-0 flex-1 rounded-xl border border-border bg-background px-3 py-2 text-sm" />
                <button type="button" onClick={() => void createCatalog()} className="rounded-xl bg-foreground px-4 py-2 text-sm font-bold text-background">Crear</button>
              </div>
              <select value={selectedCatalog} onChange={(event) => setSelectedCatalog(event.target.value)} className="mt-3 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm">
                <option value="">Seleccionar catálogo</option>
                {catalogs.map((catalog) => <option key={catalog.id} value={catalog.id}>{catalog.name} · {catalog.status}</option>)}
              </select>
              <select value={catalogAudience} onChange={(event) => setCatalogAudience(event.target.value as 'b2c' | 'b2b' | 'both')} className="mt-3 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm">
              ...
              {selectedCatalog && (
                <>
                  <p className="mt-4 text-xs text-muted-foreground">Selecciona productos del catálogo:</p>
                  <div className="mt-2 max-h-52 space-y-2 overflow-auto">
                    {products.map((product) => (
                      <label key={product.id} className="flex items-center gap-3 rounded-xl border border-border p-3 text-sm">
                        <input type="checkbox" checked={catalogItems.has(product.id)} onChange={() => void toggleCatalogItem(product.id)} />
                        <span className="truncate font-semibold">{product.title}</span>
                      </label>
                    ))}
                  </div>
                  <button type="button" onClick={() => void publishCatalog()} className="mt-4 w-full rounded-xl bg-primary px-4 py-3 text-sm font-black text-primary-foreground">Publicar catálogo en Marketplace</button>
                </>
              )}
            ...
            <section className="rounded-3xl border border-border bg-card p-6">
              ...
              <select value={selectedProductForContent} onChange={(event) => setSelectedProductForContent(event.target.value)} className="mt-4 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm">
                <option value="">Sin producto asociado</option>
                {products.map((product) => <option key={product.id} value={product.id}>{product.title}</option>)}
              </select>
              <textarea value={postText} onChange={(event) => setPostText(event.target.value)} rows={3} placeholder="Escribe una publicación…" className="mt-3 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" />
              ...
              <textarea value={storyText} onChange={(event) => setStoryText(event.target.value)} rows={2} placeholder="Escribe una historia…" className="mt-4 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" />
