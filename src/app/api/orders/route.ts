...
export async function POST(request:Request){
 ...
 try{
  ...
  const affiliateFromBody=parsed.data.affiliate_ref?.trim()||null
  const affiliateFromCookie=request.headers.get('cookie')?.match(/(?:^|;\s*)credi_affiliate_ref=([^;]*)/)?.[1]
  let affiliateRef=affiliateFromBody
  if(!affiliateRef && affiliateFromCookie){try{affiliateRef=decodeURIComponent(affiliateFromCookie).trim()||null}catch{affiliateRef=null}}
  if(affiliateRef && affiliateRef.length>128)return jsonError('La referencia de afiliado no es válida.',400,'INVALID_AFFILIATE')
  const admin=createAdminClient()
  const {data:rpcData,error:rpcError}=await admin.rpc('create_pending_order_batch',{p_buyer_id:user.id,p_items:normalizedItems,p_affiliate_ref:affiliateRef,p_region:parsed.data.region,p_idempotency_key:idempotencyKey})
