...
export async function GET(){
 ...
 const rows=items??[]; const orderIds=[...new Set(rows.map(x=>x.order_id))]; const orders=orderIds.length?(await supabase.from('orders').select('id,buyer_id,status,total_amount,created_at').in('id',orderIds).limit(10000)).data??[]:[]
 ...
 const avgStock=(products??[]).reduce((s,p)=>s+Math.max(0,Number(p.stock||0)),0); const aiRevenue=(audit??[]).filter(x=>x.status==='executed'&&x.action==='create_payment').reduce((s,x)=>{const input=x.input&&typeof x.input==='object'&&!Array.isArray(x.input)?x.input as Record<string,unknown>:{};return s+Number(input.amount||0)},0)
