...
export async function GET(){
 ...
 const rows=items??[]; const orderIds=[...new Set(rows.map(x=>x.order_id))]; const {data:orders}=orderIds.length?await supabase.from('orders').select('id,buyer_id,status,total_amount,created_at').in('id',orderIds).limit(10000):{data:[] as Array<{id:string;buyer_id:string;status:string;total_amount:number}>}
 ...
 const avgStock=(products??[]).reduce((s,p)=>s+Math.max(0,Number(p.stock||0)),0); const aiRevenue=(audit??[]).filter(x=>x.status==='executed'&&x.action==='create_payment').reduce((s,x)=>s+Number((x.input as Record<string,unknown> | null)?.amount||0),0)
