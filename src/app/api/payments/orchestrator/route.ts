...
export async function POST(request: Request) {
  ...
  const metadata = buildPaymentMetadata({ source: 'payment-orchestrator', requested_method: method, requested_at: new Date().toISOString() })
  const { data: payment, error } = await supabase.from('payment_orchestrations').insert({ user_id: auth.user.id, order_id: orderId, amount, currency, method_type: method, provider: method === 'stripe' ? 'stripe' : method === 'crypto' ? 'crypto' : method, status: method === 'manual' ? 'requires_action' : 'pending', client_reference: clientReference, idempotency_key: idempotencyKey, metadata, expires_at: expiresAt }).select('id,order_id,amount,currency,method_type,provider,status,client_reference,metadata,expires_at,created_at').single()
  if (error) {
    if (error.code === '23505') {
