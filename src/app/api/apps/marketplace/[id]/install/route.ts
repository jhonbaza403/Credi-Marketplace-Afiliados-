...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
 ...
 const requested=Array.isArray(body.scopes)?body.scopes.filter((x):x is string=>typeof x==='string').slice(0,20):[]; const manifest=app.manifest as Record<string,unknown>|null; const rawAllowed=manifest?.scopes; const allowed=Array.isArray(rawAllowed)?rawAllowed.filter((x):x is string=>typeof x==='string'):[]; const scopes=requested.length?requested.filter(s=>allowed.length?allowed.includes(s):true):allowed
