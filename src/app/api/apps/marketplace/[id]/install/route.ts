...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
 ...
 const requested=Array.isArray(body.scopes)?body.scopes.filter((x):x is string=>typeof x==='string').slice(0,20):[]; const manifest=app.manifest as {scopes?:unknown} | null; const allowed=Array.isArray(manifest?.scopes)?manifest.scopes.filter((x):x is string=>typeof x==='string'):[]; const scopes=requested.length?requested.filter(s=>allowed.length?allowed.includes(s):true):allowed
