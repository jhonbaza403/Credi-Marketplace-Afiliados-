...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
 ...
 const requested=Array.isArray(body.scopes)?body.scopes.filter((x):x is string=>typeof x==='string').slice(0,20):[]; const allowed=Array.isArray((app.manifest as Record<string, unknown> | null)?.scopes)?(app.manifest as Record<string, unknown>).scopes.filter((x:unknown)=>typeof x==='string'):[]; const scopes=requested.length?requested.filter(s=>allowed.length?allowed.includes(s):true):allowed
