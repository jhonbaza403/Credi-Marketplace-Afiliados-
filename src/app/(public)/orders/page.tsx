      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <header className="mb-8">
          ...
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Consulta el estado, importe y productos de tus compras desde una sola sección.
          </p>
        </header>
        ...
        {orders.length === 0 ? (
          <section className="rounded-3xl border border-dashed border-border bg-card p-10 text-center shadow-sm">
            ...
            <Link
              href="/marketplace"
              className="mt-6 inline-flex rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:opacity-90"
            >
              Ir al Marketplace
            </Link>
          </section>
                ...
                <article key={order.id} className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="sm:text-right">
                      <span className="inline-flex rounded-full border border-border bg-muted px-3 py-1 text-xs font-bold text-foreground">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-16 sm:px-6">
        <section className="rounded-3xl border border-destructive/20 bg-card p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            El servicio de pedidos no respondió correctamente. Puedes reintentarlo sin perder tu sesión.
          </p>
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/orders" className="rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:opacity-90">
              Reintentar
            </Link>
            <Link href="/marketplace" className="rounded-xl border border-border px-5 py-3 text-sm font-bold text-foreground hover:bg-muted">
              Marketplace
            </Link>
          </div>
        </section>
