...
-- Performance/RLS reconciliation: remove duplicate policies/indexes and use initplan-safe auth checks.
drop policy if exists affiliate_products_select_own on public.affiliate_products;
drop policy if exists affiliate_products_insert_own on public.affiliate_products;
drop policy if exists affiliate_products_update_own on public.affiliate_products;
drop policy if exists affiliate_products_delete_own on public.affiliate_products;
...
for select to authenticated
using (exists (select 1 from public.affiliates a where a.id=affiliate_products.affiliate_id and a.user_id=(select auth.uid())));
...
for insert to authenticated
with check (exists (select 1 from public.affiliates a where a.id=affiliate_products.affiliate_id and a.user_id=(select auth.uid()) and a.is_active=true));
...
for update to authenticated
using (exists (select 1 from public.affiliates a where a.id=affiliate_products.affiliate_id and a.user_id=(select auth.uid())))
with check (exists (select 1 from public.affiliates a where a.id=affiliate_products.affiliate_id and a.user_id=(select auth.uid())));
