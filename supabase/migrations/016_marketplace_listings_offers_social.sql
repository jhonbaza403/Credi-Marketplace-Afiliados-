-- ============================================================
-- 016_marketplace_listings_offers_social.sql
-- Listings, offers and social publishing foundation
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.social_accounts (
    ...
    CONSTRAINT social_accounts_status_valid CHECK (
        status IN ('active', 'expired', 'revoked', 'disconnected', 'error')
    ),
...
ON public.listing_offers FOR SELECT TO anon, authenticated
...
ON public.listing_offers FOR ALL TO authenticated
USING (auth.uid() = seller_id)
WITH CHECK (auth.uid() = seller_id);
...
ON public.social_publications FOR ALL TO authenticated
USING (auth.uid() = owner_id)
WITH CHECK (auth.uid() = owner_id);

CREATE INDEX IF NOT EXISTS idx_social_publications_social_account
ON public.social_publications(social_account_id);
CREATE INDEX IF NOT EXISTS idx_listings_offers_seller
ON public.listing_offers(seller_id);
